...
def calc_point2point(predict, actual):
    ...
    print(f'TP:{predict * actual}')
    print(f'TN:{(1 - predict) * (1 - actual)}')
...
def pot_eval(init_score, score, label, lm, q=1e-5, level=0.02):
    ...
    combined_energy = np.concatenate([init_score, score], axis=0)
    pot_th = np.percentile(combined_energy, 100 - anomaly_ratio)
    pred, p_latency = adjust_predicts(score, label, pot_th, calc_latency=True)
    p_t = calc_point2point(pred, label)
    '''
    return {
    ...
    }, np.array(pred), pot_th


'''
def pot_eval(train_energy, test_energy, label, anormly_ratio):
    """
    Run POT method on given score.
    Args:
        init_score (np.ndarray): The data to get init threshold.
    ...
    pot_th = np.percentile(combined_energy, 100 - anormly_ratio)

    print("Threshold :", pot_th)

    pred, p_latency = adjust_predicts(test_energy, label, pot_th, calc_latency=True)
